...
btnExcluir.onclick = function(){
    for (let i = 0; i < BD.length; i++){
        ...
        if (elemento.checked){
            ...
            tabela.innerHTML = `<tr><td width="30px"></td><td>Nome</td><td>Quant.</td><td>Preço</td><td>prateleira</td><td>descrição</td><td>categoria</td></tr>`;
            montarTabela();
        }
    }
}
...
function montarTabela(){
    for (let i = 0; i < BD.length; i++){
        tabela.innerHTML += `<tr><td width="30px"><input type="checkbox" id="${i}" onchange="verificar()"></td><td>${BD[i].nome}</td><td>${BD[i].quantidade}</td><td>${BD[i].preco}</td><td>${BD[i].prateleira}</td><td>${BD[i].descricao}</td></tr>`;
    }
}
